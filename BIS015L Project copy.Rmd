...
top_5_diseases %>%
  ggplot(aes(x=reorder(disease, all_cases), y= all_cases))+
...
disease_data %>%
  ...
  summarise(total_cases = sum(cases))
```
Alpine county only has 2 total cases! The population is super small with only 1,175 as of the 2019 census.

5. Does population affect case numbers?
...
6. Are male or females more impacted by disease in California
```{r}
disease_data %>%
...
disease_data %>% 
  ...
  scale_fill_manual(values = palette2) +
    labs(title="Male vs Female Disease Prevalence",
       ...
       y="Number of Cases")
```
It seems like males are more impacted by disease than females!
...
#Yes, so we cannot say increase in disease is due to disease factors alone, there are also way more people! 
#We can take the proportion of disease to the population over time to see if there was an actual disease incidence increase.
