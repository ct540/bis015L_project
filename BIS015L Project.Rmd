...
5. Are male or females more impacted by disease in California
```{r}
disease_data %>%
...
6. Scaling disease per county
```{r}
disease_data %>%
  filter(county!="California", sex=="Total", year == "2019") %>%
  group_by(county) %>%
  summarise(sum_cases = sum(cases)) %>%
  mutate(scaling_cases = (sum_cases / population)) %>%
  arrange(scaling_cases)
...
disease_data %>%
  ...
  arrange(scaling_cases)
```
7. Does population affect case numbers?
...
```
It seems like when there is a higher population there is a higher number of cases and vice versa, which is expected.
