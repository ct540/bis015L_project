...
disease_data %>%
  filter(county!="California", sex=="Total") %>%
  group_by(county) %>%
  mutate(scaling_cases = (mean(cases) / population))
  arrange(desc(scaling_cases))

```
6. How has disease prevelance changed over time?
...
```{r}
cases_overall %>%
  ggplot(aes(x=population, y= sum_cases))+
  geom_line() +
  geom_point(size = 1, na.rm = T) +
  theme(axis.text.x = element_text(hjust = 1), plot.title = element_text(size = rel(2), hjust = .5)) +
  labs(title = "Top 5 Counties Disease Growth",
       x = "Year",
       y= "Income")
